...
class ConfigOption
{
  ...
  public:
    ...
    QByteArray docs() const { return m_doc; }

    QByteArray dependsOn() const { return m_dependency; }
    void addDependency(const char *dep) { m_dependency = dep; }
    void setEncoding(const QByteArray &e) { m_encoding = e; }
...
class ConfigInt : public ConfigOption
{
  public:
    ...
    QByteArray *valueStringRef() { return &m_valueString; }
...
class Config
{
  public:
    ...
    {
      QString result=m_userComment;
      m_userComment.resize(0);

      return result.replace(QRegExp("\r"),"").toUtf8();
    }
