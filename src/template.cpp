                               ...
                               bool allowEmptyEntries = false, bool cleanup = true)
{
   ...
   while (i != -1) {
      if ( str.mid(j, i - j).length() > 0 ) {
         if (cleanup) {
            lst.append(str.mid(j, i - j).trimmed());
         } else {
...
TemplateVariant::TemplateVariant(const QByteArray &s, bool raw)
{
   p = new Private(String);
   p->strVal = s;
   p->raw = raw;
...
TemplateVariant &TemplateVariant::operator=(const TemplateVariant &v)
{
   ...
   if      (tmpType == Struct && tmpStruct) {
      tmpStruct->release();
   } else if (tmpType == List   && tmpList  ) {
      tmpList->release();
   }
   return *this;
}

QByteArray TemplateVariant::toString() const
{
   QByteArray result;
   switch (p->type) {
      case None:
      ...
      case Bool:
         ...
         break;
      case Integer:
         result = QByteArray().setNum(p->intVal);
         break;
      case String:
         result = p->strVal;
         break;
...
TemplateVariant TemplateVariant::call(const QList<TemplateVariant> &args)
{
   if (p->type == Function) {
...
QByteArray TemplateVariant::typeAsString() const
{
   switch (p->type) {
      case None:
         return "none";
...
class TemplateBlockContext
{
 public:
   ...
   TemplateNodeBlock *get(const QByteArray &name) const;
   TemplateNodeBlock *pop(const QByteArray &name) const;
 ...
 private:
   QHash<QString,  QList<TemplateNodeBlock> > m_blocks;
};

/** @brief A container to store a key-value pair */
struct TemplateKeyValue {
   TemplateKeyValue() {}
   TemplateKeyValue(const QByteArray &k, const TemplateVariant &v) : key(k), value(v) {}
   QByteArray key;
...
class TemplateContextImpl : public TemplateContext
{
 public:
   ...
   TemplateVariant get(const QByteArray &name) const;
   const TemplateVariant *getRef(const QByteArray &name) const;
   void setOutputDirectory(const QByteArray &dir) {
      m_outputDir = dir;
   }
   void setEscapeIntf(const QByteArray &ext, TemplateEscapeIntf *intf) {
      int i = (!ext.isEmpty() && ext.at(0) == '.') ? 1 : 0;
      m_escapeIntfDict.insert(ext.mid(i), new TemplateEscapeIntf * (intf));
   }
   void selectEscapeIntf(const QByteArray &ext) {
   ...
   TemplateVariant getPrimary(const QByteArray &name) const;
   void setLocation(const QByteArray &templateName, int line) {
      m_templateName = templateName;
      m_line = line;
   }
   QByteArray templateName() const                {
      return m_templateName;
   }
   int line() const                             {
      return m_line;
   }
   QByteArray outputDirectory() const             {
      return m_outputDir;
   }
   TemplateEscapeIntf *escapeIntf() const       {
      return m_activeEscapeIntf;
   }
   TemplateSpacelessIntf *spacelessIntf() const {
      return m_spacelessIntf;
   }
   void enableSpaceless(bool b)                 {
      if (b && !m_spacelessEnabled) {
         m_spacelessIntf->reset();
      }
      m_spacelessEnabled = b;
   }
   bool spacelessEnabled() const                {
      return m_spacelessEnabled && m_spacelessIntf;
   }
   void warn(const char *fileName, int line, const char *fmt, ...) const;

   // index related functions
   void openSubIndex(const QByteArray &indexName);
   void closeSubIndex(const QByteArray &indexName);
   void addIndexEntry(const QByteArray &indexName, const QList<TemplateKeyValue> &arguments);
 ...
 private:
   ...
   QList< QHash<QString, TemplateVariant> > m_contextStack;

   TemplateBlockContext m_blockContext;
   QHash<QString, TemplateEscapeIntf *> m_escapeIntfDict;
...
{
   struct ListElem {
      ListElem(const QByteArray &k, const TemplateVariant &v) : key(k), value(v) {}
      QByteArray key;
      TemplateVariant value;
   };
 ...
 private:
   static QByteArray determineSortKey(TemplateStructIntf *s, const QByteArray &arg) {
      int i, p = 0;
      QByteArray result;
      while ((i = arg.find("{{", p)) != -1) {
         ...
         int j = arg.find("}}", i + 2);
         if (j != -1) {
            QByteArray var = arg.mid(i + 2, j - i - 2);
...
class FilterGroupBy
{
 ...
 private:
   static QByteArray determineSortKey(TemplateStructIntf *s, const QByteArray &attribName) {
...
class FilterAlphaIndex
{
 private:
   ...
   static uint determineSortKey(TemplateStructIntf *s, const QByteArray &attribName) {
      TemplateVariant v = s->get(attribName);
...
class TemplateFilterFactory
{
 public:
   ...
   static TemplateFilterFactory *instance() {
      ...
      if (instance == 0) {
         instance = new TemplateFilterFactory;
      }
      return instance;
   }

   TemplateVariant apply(const QByteArray &name, const TemplateVariant &v, const TemplateVariant &arg, bool &ok) {
   ...
   void registerFilter(const QByteArray &name, FilterFunction *func) {
...
class ExprAstNumber : public ExprAst
{
 public:
   ...
   virtual TemplateVariant resolve(TemplateContext *) {
      return TemplateVariant(m_number);
   }
 private:
   int m_number;
};
...
class ExprAstLiteral : public ExprAst
{
 public:
   ...
   const QByteArray &literal() const {
      return m_literal;
   }
   virtual TemplateVariant resolve(TemplateContext *) {
      return TemplateVariant(m_literal);
   }
 private:
   QByteArray m_literal;
};
...
class ExprAstBinary : public ExprAst
{
 public:
   ...
   virtual TemplateVariant resolve(TemplateContext *c) {
      TemplateContextImpl *ci = dynamic_cast<TemplateContextImpl *>(c);
      if (ci == 0) {
         return TemplateVariant();   // should not happen
      }
...
class TemplateParser
{
 public:
   TemplateParser(const TemplateEngine *engine,
                  const QByteArray &templateName, QList<TemplateToken> &tokens);
   void parse(TemplateNode *parent, int line, const QStringList &stopAt,
...
class ExpressionParser
{
 ...
 private:
   ...
   ExprAstFilter *parseFilter() {
      TRACE(("{parseFilter(%s)\n", m_curToken.id.data()));
      QByteArray filterName = m_curToken.id;
      getNextToken();
      ExprAst *argExpr = 0;
      if (m_curToken.type == ExprToken::Operator &&
            m_curToken.op == Operator::Colon) {
...
{
 public:
   TemplateImpl(TemplateEngine *e, const QByteArray &name, const char *data, int size);
   void render(FTextStream &ts, TemplateContext *c);
...
const TemplateVariant *TemplateContextImpl::getRef(const QByteArray &name) const
{
   QListIterator< QHash<QString, TemplateVariant> > it(m_contextStack);
   QHash<QString, TemplateVariant> *dict;
...
TemplateVariant TemplateContextImpl::getPrimary(const QByteArray &name) const
{
...
{
   QHash<QString, TemplateVariant> *dict = new QHash<QString, TemplateVariant>;
   
   // CS BROOM  dict->setAutoDelete(true);
...
void TemplateContextImpl::openSubIndex(const QByteArray &indexName)
{
...
void TemplateContextImpl::closeSubIndex(const QByteArray &indexName)
{
...
static TemplateVariant getPathFunc(const void *ctx, const QList<TemplateVariant> &)
{
...
void TemplateContextImpl::addIndexEntry(const QByteArray &indexName, const QList<TemplateKeyValue> &arguments)
{
   QListConstIterator<TemplateKeyValue> it = arguments.begin();
   ...
   if (! stack) { // no stack yet, create it!
      ...
      // CS BROOM  stack->setAutoDelete(true);
...
{
 public:
   TemplateNodeText(TemplateParser *, TemplateNode *parent, int, const QByteArray &data)
...
{
 public:
   TemplateNodeVariable(TemplateParser *parser, TemplateNode *parent, int line, const QByteArray &var)
   ...
   void render(FTextStream &ts, TemplateContext *c) {
      ...
      if (m_var) {
         TemplateVariant v = m_var->resolve(c);
         if (v.type() == TemplateVariant::Function) {
            v = v.call(QList<TemplateVariant>());
         }
...
template<class T> class TemplateNodeCreator : public TemplateNode
{
 ...
 protected:
   void mkpath(TemplateContextImpl *ci, const QByteArray &fileName) {
      int i = fileName.find('/');
      QByteArray outputDir = ci->outputDirectory();
      QDir d(outputDir);
      if (!d.exists()) {
         QDir rootDir;
         rootDir.setPath(QDir::currentPath());
      ...
      while (i != -1) { // fileName contains path part
         if (d.exists()) {
            ...
            QByteArray dirName = outputDir + '/' + fileName.left(i);
            d = QDir(dirName);
            j = i + 1;
         }
         i = fileName.find('/', i + 1);
...
{
 public:
   TemplateNodeIf(TemplateParser *parser, TemplateNode *parent, int line, const QByteArray &data) :
      TemplateNodeCreator<TemplateNodeIf>(parser, parent, line) {

      // CS BROOM   m_ifGuardedNodes.setAutoDelete(true);
...
{
 public:
   TemplateNodeRepeat(TemplateParser *parser, TemplateNode *parent, int line, const QByteArray &data)
...
{
 public:
   TemplateNodeRange(TemplateParser *parser, TemplateNode *parent, int line, const QByteArray &data)
      ...
      if (i1 == -1) {
         ...
         m_var = data.left(i1).trimmed();
         if (m_var.isEmpty()) {
            parser->warn(m_templateName, line, "range needs an iterator variable");
         }
         start = data.mid(i1 + 6, i2 - i1 - 6).trimmed();
         if (i2 != -1) {
            end = data.right(data.length() - i2 - 4).trimmed();
            m_down = false;
         } else if (i3 != -1) {
            end = data.right(data.length() - i3 - 8).trimmed();
            m_down = true;
         }
      }
      ExpressionParser expParser(parser, line);
...
{
 public:
   TemplateNodeFor(TemplateParser *parser, TemplateNode *parent, int line, const QByteArray &data)
...
{
 public:
   TemplateNodeMsg(TemplateParser *parser, TemplateNode *parent, int line, const QByteArray &)
...
{
 public:
   TemplateNodeBlock(TemplateParser *parser, TemplateNode *parent, int line, const QByteArray &data)
...
{
 public:
   TemplateNodeExtend(TemplateParser *parser, TemplateNode *parent, int line, const QByteArray &data)
   ...
   void render(FTextStream &ts, TemplateContext *c) {
      ...
      QByteArray extendFile = m_extendExpr->resolve(c).toString();
...
{
 public:
   TemplateNodeInclude(TemplateParser *parser, TemplateNode *parent, int line, const QByteArray &data)
   ...
   void render(FTextStream &ts, TemplateContext *c) {
      ...
      ci->setLocation(m_templateName, m_line);
      if (m_includeExpr) {
         QByteArray includeFile = m_includeExpr->resolve(c).toString();
 ...
 public:
   TemplateNodeCreate(TemplateParser *parser, TemplateNode *parent, int line, const QByteArray &data)
      ...
      if (i == -1) {
         ...
         ExpressionParser ep(parser, line);
         m_fileExpr = ep.parse(data.left(i).trimmed());
         m_templateExpr = ep.parse(data.mid(i + 6).trimmed());
      }
   }
   ~TemplateNodeCreate() {
   ...
   void render(FTextStream &, TemplateContext *c) {
      ...
      if (m_templateExpr && m_fileExpr) {
         QByteArray templateFile = m_templateExpr->resolve(c).toString();
         QByteArray outputFile = m_fileExpr->resolve(c).toString();
         if (templateFile.isEmpty()) {
            ...
            if (t) {
               ...
               if (createTemplate) {
                  ...
                  QByteArray extension = outputFile;
...
class TemplateNodeTree : public TemplateNodeCreator<TemplateNodeTree>
{
 ...
 public:
   ...
   static TemplateVariant renderChildrenStub(const void *ctx, const QList<TemplateVariant> &) {
      return TemplateVariant(((TreeContext *)ctx)->object->
                             renderChildren((const TreeContext *)ctx), true);
   }
   QByteArray renderChildren(const TreeContext *ctx) {
...
{
 public:
   TemplateNodeOpenSubIndex(TemplateParser *parser, TemplateNode *parent, int line, const QByteArray &data)
...
{
 public:
   TemplateNodeCloseSubIndex(TemplateParser *parser, TemplateNode *parent, int line, const QByteArray &data)
...
class TemplateNodeSet : public TemplateNodeCreator<TemplateNodeSet>
{
 ...
 public:
   TemplateNodeSet(TemplateParser *parser, TemplateNode *parent, int line, const QByteArray &data)
...
{
 public:
   TemplateNodeSpaceless(TemplateParser *parser, TemplateNode *parent, int line, const QByteArray &)
...
{
 public:
   TemplateNodeMarkers(TemplateParser *parser, TemplateNode *parent, int line, const QByteArray &data)
      ...
      if (!m_var.isEmpty() && m_listExpr && m_patternExpr) {
         ...
         if (list) {
            if (patternStr.type() == TemplateVariant::String) {
               TemplateListIntf::ConstIterator *it = list->createIterator();
               c->push();
               QByteArray str = patternStr.toString();
...
{
 public:
   TemplateNodeResource(TemplateParser *parser, TemplateNode *parent, int line, const QByteArray &data)
   ...
   void render(FTextStream &, TemplateContext *c) {
      ...
      ci->setLocation(m_templateName, m_line);
      if (m_resExpr) {
         QByteArray resourceFile = m_resExpr->resolve(c).toString();
         if (resourceFile.isEmpty()) {
            ci->warn(m_templateName, m_line, "invalid parameter for resource command\n");
         } else {
            QByteArray outputDirectory = ci->outputDirectory();
            if (m_asExpr) {
               QByteArray targetFile = m_asExpr->resolve(c).toString();
...
class TemplateNodeFactory
{
 public:
   ...
   static TemplateNodeFactory *instance() {
      ...
      if (instance == 0) {
         instance = new TemplateNodeFactory;
      }
      return instance;
   }
   ...
   void registerTemplateNode(const QByteArray &name, CreateFunc func) {
      m_registry.insert(name, (void *)func);
   }
...
TemplateNodeBlock *TemplateBlockContext::pop(const QByteArray &name) const
{
   QList<TemplateNodeBlock> *list = m_blocks.find(name);
   if (list == 0 || list->count() == 0) {
      return 0;
...
{
   QHashIterator<QString, QList<TemplateNodeBlock> > di(ctx->m_blocks);
...
TemplateLexer::TemplateLexer(const TemplateEngine *engine, const QByteArray &fileName, const char *data, int size) :
...
void TemplateLexer::tokenize(QList<TemplateToken> &tokens)
{
   ...
   for (; (c = *p); p++, pos++) {
      ...
      if (c == '\n') { // new line
         ...
         if (markStartPos != -1 && emptyOutputLine) {
            lastTokenPos = startLinePos;
         }
         // reset markers
         markStartPos = -1;
         line++;
...
void TemplateParser::parse(
   TemplateNode *parent, int line, const QStringList &stopAt,
   QList<TemplateNode> &nodes)
{
...
TemplateImpl::TemplateImpl(TemplateEngine *engine, const QByteArray &name, const char *data, int size)
   : TemplateNode(0)
{
   m_name = name;
   m_engine = engine;
...
class TemplateEngine::Private
{
 ...
 public:
   ...
   void enterBlock(const QByteArray &fileName, const QByteArray &blockName, int line) {
      //for (int i=0;i<m_indent;i++) printf("  ");
      //m_indent++;
 ...
 private:
   QHash<QString, Template> m_templateCache;
...
Template *TemplateEngine::loadByName(const QByteArray &fileName, int line)
{
...
void TemplateEngine::enterBlock(const QByteArray &fileName, const QByteArray &blockName, int line)
{
