...
#define DOC_ERROR(msg) \
    warn_doc_error(m_fileName, lineNumber(), msg)
...
class DBusXMLHandler : public QXmlDefaultHandler
{
 public:
   ...
   bool startElement(const QString &namespaceURI, const QString &localName, const QString &qName, const QXmlAttributes &attributes) {
      ...
      if (DBUS("node")) {
         ...
         COND_DOC_ERROR(idx < 0, "Anonymous node found.");

         m_currentNode = attributes.value(idx).toUtf8();
      ...
      if (DBUS("interface")) {
         ...
         const int idx(indexOf(attributes, "name"));
         COND_DOC_ERROR(idx < 0, "Interface without a name found.");
         ...
         m_currentInterface->name = substitute(attributes.value(idx).toUtf8(), ".", "::");
      ...
      if (DBUS("method") || DBUS("signal")) {
         ...
         const int idx(indexOf(attributes, "name"));
         COND_DOC_ERROR(idx < 0, "Method or signal without a name found.");

         m_currentMethod = createEntry();

         m_currentMethod->section = Entry::FUNCTION_SEC;
         m_currentMethod->name = attributes.value(idx).toUtf8();
      ...
      if (DBUS("arg")) {
         ...
         COND_DOC_ERROR(name_idx < 0, "Argument without a name found.");
         COND_DOC_ERROR(!hasType(attributes), "Argument without a type found.");
         ...
         m_currentArgument->type = getType(attributes).toUtf8();
         m_currentArgument->name = attributes.value(name_idx).toUtf8();
         if (direction_idx >= 0) {
            m_currentArgument->attrib = attributes.value(direction_idx).toUtf8();
         } else {
            if (m_currentMethod->mtype == Signal) {
      ...
      if (DBUS("property")) {
         ...
         const int name_idx(indexOf(attributes, "name"));
         COND_DOC_ERROR(name_idx < 0, "Anonymous property found.");
         COND_DOC_ERROR(!hasType(attributes), "Property without a type found.");

         const int access_idx(indexOf(attributes, "access"));
         COND_DOC_ERROR(access_idx < 0, "Property without a access attribute found.");

         COND_DOC_ERROR(attributes.value(access_idx) != "read" &&
                        attributes.value(access_idx) != "write" &&
                        attributes.value(access_idx) != "readwrite",
                        QString("Property with invalid access attribute \"%1\" found.").arg(attributes.value(access_idx)));
         ...
         m_currentProperty->name = attributes.value(name_idx).toUtf8();
         m_currentProperty->mtype = Property;
         m_currentProperty->type = getType(attributes).toUtf8();
      }
      ...
      if (EXTENSION("member")) {
         ...
         const int name_idx(indexOf(attributes, "name"));
         COND_DOC_ERROR(name_idx < 0, "Anonymous member found.");
         COND_DOC_ERROR(!hasType(attributes), "Member without a type found.");

         createEntry();

         m_currentEntry->section = Entry::VARIABLE_SEC;
         m_currentEntry->name = attributes.value(name_idx).toUtf8();
      ...
      if (EXTENSION("enum") || EXTENSION("flagset")) {
         ...
         const int name_idx(indexOf(attributes, "name"));
         COND_DOC_ERROR(name_idx < 0, "Anonymous enum found.");
         ...
         m_currentEnum->name = attributes.value(name_idx).toUtf8();

         openScopes(m_currentEnum);

         m_currentEnum->type = m_currentEntry->name + " enum";

         addNamedType(type.toUtf8());
      }
      ...
      if (EXTENSION("value")) {
         ...
         const int name_idx(indexOf(attributes, "name"));
         COND_DOC_ERROR(name_idx < 0, "Anonymous value found.");
         ...
         if (value_idx >= 0) {
            m_currentEntry->initializer = attributes.value(value_idx).toUtf8();
         }
      }
   ...
   bool endElement(const QString &namespaceURI, const QString &localName, const QString &qName) {
      ...
      if (EXTENSION("struct")) {
         ...
         addNamedType(current_type.toUtf8());
   ...
   bool comment(const QString &comment_) {
      ...
      m_currentComment = new CommentData(m_fileName, lineNumber(), comment_.toUtf8());
   ...
   void handleComment() {
      ...
      QByteArray text(m_currentComment->text);
 ...
 private:
   ...
   QString getType(const QXmlAttributes &attributes) {
         ...
         if (m_namedTypeMap.contains(type)) {
            if (type_idx >= 0) {
               ...
               if (dbus_type != m_namedTypeMap[type]) {
                  DOC_ERROR(QString("Type \"%1\" does not match up with previous definition of named type \"%2\" (which was \"%3\".").
                            arg(dbus_type).arg(type).arg(m_namedTypeMap[type]));
               }
            }
            return type;
         }
      ...
      if (type_idx >= 0) {
         type = attributes.value(type_idx).toUtf8();

         QRegExp reg_exp(QByteArray("(a?[ybnqiuxdtsogv]|a[{]sv[}])"));
   ...
   QString getDBusType(const QByteArray &type) {
      QByteArray scoped_type = type;
   ...
   void addNamedType(const QByteArray &type) {
   ...
   int indexOf(const QXmlAttributes &attributes, const QString &name,
      ...
      if (attributes.type(idx) != type) {
         return -1;
      }
      if (mandatory && attributes.value(idx).isEmpty()) {
         return -1;
      }
   ...
   Entry *openNamespace(const QString &name) {
      ...
      scoped_name.append(name.toUtf8());
   ...
   struct ElementData {
      ElementData(const QByteArray &e) :
         element(e) {
      }
      ~ElementData() { }
   ...
   struct CommentData {
      CommentData(const QByteArray &f, const int l, const QByteArray &t) :
         isJavaStyle(false),
         isQtStyle(false),
         line(l),
         fileName(f) {
   ...
   QMap<QByteArray, QByteArray> m_namedTypeMap;
};
                                ...
                                QStringList & /*filesInSameTranslationUnit*/)
{
   ...
   if (! handler.errorString().isEmpty()) {
      err("DBus XML Parser: Error at line %d: %s\n", handler.locator()->lineNumber(), handler.errorString().toUtf8().data());
   }

   groupLeaveFile(fileName, 1);
}

bool DBusXMLScanner::needsPreprocessing(const QByteArray & /* extension */)
{
   return (false);
}
