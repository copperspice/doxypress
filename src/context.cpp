...
class PropertyMapper
{
 ...
 public:
    ...
    */
   template<typename T>
   void addProperty(const char *name, const T *obj, typename PropertyFunc<T>::Handler handle) {
 ...
 private:
   QHash<QString, PropertyFuncIntf *> m_map;
};
...
class ConfigContext::Private
{
 public:
   ...
   TemplateVariant fetchList(const QByteArray &name, const QStringList *list) {
 ...
 private:
   QHash<QString, TemplateVariant> m_cachedLists;
};
//%% }
...
class TranslateContext::Private : public PropertyMapper
{
 public:
   ...
   TemplateVariant handleGeneratedAt(const QList<TemplateVariant> &args) const {
      if (args.count() == 2) {
   ...
   TemplateVariant handleInheritanceDiagramFor(const QList<TemplateVariant> &args) const {
      if (args.count() == 1) {
   ...
   TemplateVariant handleCollaborationDiagramFor(const QList<TemplateVariant> &args) const {
      if (args.count() == 1) {
   ...
   TemplateVariant handleInheritsList(const QList<TemplateVariant> &args) const {
      if (args.count() == 1) {
   ...
   TemplateVariant handleInheritedByList(const QList<TemplateVariant> &args) const {
      if (args.count() == 1) {
   ...
   TemplateVariant handleWriteList(const QList<TemplateVariant> &args) const {
      if (args.count() == 1) {
   ...
   TemplateVariant handleImplementedBy(const QList<TemplateVariant> &args) const {
      if (args.count() == 1) {
   ...
   TemplateVariant handleReimplementedBy(const QList<TemplateVariant> &args) const {
      if (args.count() == 1) {
   ...
   TemplateVariant handleSourceRefs(const QList<TemplateVariant> &args) const {
      if (args.count() == 1) {
   ...
   TemplateVariant handleSourceRefBys(const QList<TemplateVariant> &args) const {
      if (args.count() == 1) {
   ...
   TemplateVariant handleIncludeDependencyGraph(const QList<TemplateVariant> &args) const {
      if (args.count() == 1) {
...
static TemplateVariant parseDoc(Definition *def, const QByteArray &file, int line,
                                const QByteArray &relPath, const QByteArray &docStr, bool isBrief)
{
   TemplateVariant result;
...
static TemplateVariant parseCode(MemberDef *md, const QByteArray &scopeName, const QByteArray &relPath,
                                 const QByteArray &code, int startLine = -1, int endLine = -1, bool showLineNumbers = false)
{
...
static TemplateVariant parseCode(FileDef *fd, const QByteArray &relPath)
{
 ...
 public:
   DefinitionContext(Definition *d) : m_def(d) {
      ...
      if (m_def && !m_def->getSourceFileBase().isEmpty()) {
         ...
         m_cache.fileLink->set("anchor", QByteArray());
   ...
   QByteArray relPathAsString() const {
      static bool createSubdirs = Config_getBool("CREATE_SUBDIRS");
      return createSubdirs ? QByteArray("../../") : QByteArray("");
   }
 ...
 public:
   ...
   TemplateVariant inheritanceDiagram() const {
      ...
      if (haveDot && (classDiagrams || classGraph)) {
         ...
         FTextStream t(&result);
         QByteArray name = convertToHtml(m_classDef->displayName());
   ...
   TemplateVariant memberGroups() const {
      if (!m_cache.memberGroups) {
         if (m_classDef->getMemberGroupSDict()) {
            m_cache.memberGroups.reset(MemberGroupListContext::alloc(m_classDef, relPathAsString(), m_classDef->getMemberGroupSDict(),
...
class NamespaceContext::Private : public DefinitionContext<NamespaceContext::Private>
{
 public:
   ...
   TemplateVariant memberGroups() const {
      if (!m_cache.memberGroups) {
         if (m_namespaceDef->getMemberGroupSDict()) {
            m_cache.memberGroups.reset(MemberGroupListContext::alloc(m_namespaceDef, relPathAsString(), m_namespaceDef->getMemberGroupSDict(),
   ...
   TemplateVariant detailedFunctions() const {
      QByteArray title = theTranslator->trFunctionDocumentation();
      SrcLangExt lang = m_namespaceDef->getLanguage();
      if (lang == SrcLangExt_Fortran) {
...
{
 public:
   TextGeneratorHtml(FTextStream &ts, const QByteArray &relPath)
...
class TextGeneratorFactory
{
 public:
   ...
   TextGeneratorIntf *create(FTextStream &ts, const QByteArray &relPath) {
...
TemplateVariant createLinkedText(Definition *def, const QByteArray &relPath, const QByteArray &text)
{
   QByteArray s;
   FTextStream ts(&s);
...
class MemberContext::Private : public DefinitionContext<MemberContext::Private>
{
 public:
   Private(MemberDef *md) : DefinitionContext<MemberContext::Private>(md) , m_memberDef(md) {
      ...
      if (md && md->isProperty()) {
         if (md->isGettable()) {
            m_cache.propertyAttrs->append("get");
         }
         if (md->isPrivateGettable()) {
            m_cache.propertyAttrs->append("private get");
         }
         if (md->isProtectedGettable()) {
            m_cache.propertyAttrs->append("protected get");
         }
         if (md->isSettable()) {
            m_cache.propertyAttrs->append("set");
         }
         if (md->isPrivateSettable()) {
            m_cache.propertyAttrs->append("private set");
         }
         if (md->isProtectedSettable()) {
            m_cache.propertyAttrs->append("protected set");
         }
      }
      m_cache.eventAttrs.reset(TemplateList::alloc());
      if (md && md->isEvent()) {
         if (md->isAddable()) {
            m_cache.eventAttrs->append("add");
         }
         if (md->isRemovable()) {
            m_cache.eventAttrs->append("remove");
         }
         if (md->isRaisable()) {
            m_cache.eventAttrs->append("raise");
         }
      }
   }
   virtual ~Private() {}
   ...
   TemplateVariant isAnonymous() const {
      QByteArray name = m_memberDef->name();
   ...
   TemplateVariant templateAlias() const {
      if (m_memberDef->isAlias()) {
         return createLinkedText(m_memberDef, relPathAsString(),
                                 QByteArray(" = ") + m_memberDef->typeString());
      }
      return "";
   }
   ...
   TemplateVariant memberGroups() const {
      if (!m_cache.memberGroups) {
         if (m_groupDef->getMemberGroupSDict()) {
            m_cache.memberGroups.reset(MemberGroupListContext::alloc(m_groupDef, relPathAsString(), m_groupDef->getMemberGroupSDict(),
...
class NestingNodeContext::Private : public PropertyMapper
{
 public:
   ...
   TemplateVariant id() const {
      ...
      result += QByteArray().setNum(m_index) + "_";
      return result;
   }
   TemplateVariant name() const {
      return m_def->displayName(false);
...
QByteArray NestingNodeContext::id() const
{
...
class NestingContext::Private : public GenericNodeListContext
{
 public:
   ...
   void addDirs(const DirSDict &dirDict) {
      StringMap<QSharedPointer<DirDef>>::Iterator dli(dirDict);
      DirDef *dd;
   ...
   void addPages(const PageSDict &pages, bool rootOnly) {
      StringMap<QSharedPointer<PageDef>>::Iterator pli(pages);
      PageDef *pd;
...
class NavPathElemContext::Private : public PropertyMapper
{
 public:
   ...
   TemplateVariant text() const {
      ...
      QByteArray text = m_def->localName();
...
{
 public:
   Private(ClassDef *cd, const QByteArray &name) : m_classDef(cd), m_name(name) {
      addProperty("class", this, &Private::getClass);
      addProperty("name", this, &Private::name);
   }
   TemplateVariant getClass() const {
...
InheritanceNodeContext::InheritanceNodeContext(ClassDef *cd, const QByteArray &name) : RefCountedContext("InheritanceNodeContext")
{
...
{
 public:
   void addClass(ClassDef *cd, const QByteArray &name) {
...
{
 public:
   Private(Definition *def, const QByteArray &relPath, const MemberGroup *mg) :
      m_def(def),
      m_relPath(relPath),
      m_memberGroup(mg) {
      addProperty("members",      this, &Private::members);
   ...
   TemplateVariant docs() const {
      if (!m_cache.docs) {
         QByteArray docs = m_memberGroup->documentation();
         if (!docs.isEmpty()) {
...
MemberGroupInfoContext::MemberGroupInfoContext(Definition *def,
      const QByteArray &relPath, const MemberGroup *mg) : RefCountedContext("MemberGroupInfoContext")
{
...
{
 public:
   void addMemberGroup(Definition *def, const QByteArray &relPath, const MemberGroup *mg) {
...
MemberGroupListContext::MemberGroupListContext(Definition *def, const QByteArray &relPath, const MemberGroupSDict *dict,
      bool subGrouping) : RefCountedContext("MemberGroupListContext")
{
   p = new Private;
   if (dict) {
...
{
 public:
   Private(Definition *def, const QByteArray &relPath, const MemberList *ml, const QByteArray &title, const QByteArray &subtitle) :
      m_def(def),
      m_memberList(ml),
      m_relPath(relPath),
      m_title(title),
...
MemberListInfoContext::MemberListInfoContext(
   Definition *def, const QByteArray &relPath, const MemberList *ml,
   const QByteArray &title, const QByteArray &subtitle) : RefCountedContext("MemberListInfoContext")
{
...
{
 public:
   Private(ClassDef *cd, MemberList *ml, const QByteArray &title)
...
InheritedMemberInfoContext::InheritedMemberInfoContext(ClassDef *cd, MemberList *ml,
      const QByteArray &title) : RefCountedContext("InheritedMemberInfoContext")
{
...
class InheritedMemberInfoListContext::Private : public GenericNodeListContext
{
 public:
   ...
   void addInheritedMembers(ClassDef *inheritedFrom, ClassDef *cd, MemberListType lt,
                            MemberListType lt1, int lt2, const QByteArray &title, bool additionalList) {
      int count = cd->countMembersIncludingGrouped(lt1, inheritedFrom, additionalList);

      if (lt2 != -1) {
         count += cd->countMembersIncludingGrouped((MemberListType)lt2, inheritedFrom, additionalList);
      }
...
void InheritedMemberInfoListContext::addMemberList(
   ClassDef *cd, MemberListType lt, const QByteArray &title, bool additionalList)
{
...
{
 public:
   Private(const Argument *arg, Definition *def, const QByteArray &relPath) :
...
ArgumentContext::ArgumentContext(const Argument *al, Definition *def, const QByteArray &relPath) : RefCountedContext("ArgumentContext")
{
...
{
 public:
   void addArgument(const Argument *arg, Definition *def, const QByteArray &relPath) {
...
class HtmlSpaceless : public TemplateSpacelessIntf
{
 public:
   ...
   QByteArray remove(const QByteArray &s) {
      QByteArray result;
      const char *p = s.data();
      char c;
      while ((c = *p++)) {
