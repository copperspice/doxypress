 ...
 public:
   ...
   class Delegate
   {
    public:
      ...
      typedef TemplateVariant (*StubType)(const void *obj, const QList<TemplateVariant> &args);
      ...
      TemplateVariant operator()(const QList<TemplateVariant> &args) const {
    ...
    private:
      ...
      template <class T, TemplateVariant (T::*TMethod)(const QList<TemplateVariant> &) const>
      static TemplateVariant methodStub(const void *objectPtr, const QList<TemplateVariant> &args) {
         T *p = (T *)(objectPtr);
   ...
   /** Constructs a new variant with a string value \a s. */
   TemplateVariant(const QByteArray &s, bool raw = false);
    ...
    */
   TemplateVariant call(const QList<TemplateVariant> &args);
...
class TemplateContext
{
 public:
    ...
    *  key was not found.
    */
   virtual TemplateVariant get(const QByteArray &name) const = 0;
    ...
    */
   virtual const TemplateVariant *getRef(const QByteArray &name) const = 0;

   /** When files are create (i.e. by {% create ... %}) they written
    *  to the directory \a dir.
    */
   virtual void setOutputDirectory(const QByteArray &dir) = 0;

   /** Sets the interface that will be used for escaping the result
    *  of variable expansion before writing it to the output.
    */
   virtual void setEscapeIntf(const QByteArray &extension, TemplateEscapeIntf *intf) = 0;
...
class TemplateEngine
{
 public:
    ...
    */
   Template *loadByName(const QByteArray &fileName, int fromLine);
