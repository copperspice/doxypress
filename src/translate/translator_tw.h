...
class TranslatorChinesetraditional : public Translator
{
 public:
   ...
   virtual QByteArray trGeneratedAt(const char *date, const char *projName) {
      QByteArray result = (QByteArray)"產生日期:" + date;
      if (projName) {
         result += (QByteArray)", 專案:" + projName;
      }
      result += (QByteArray)", 產生器:";
      return result;
   }

   /*! this text is put before a class diagram */
   virtual QByteArray trClassDiagram(const char *clName) {
      return (QByteArray)"類別" + clName + "的繼承圖:";
   }
   ...
   virtual QByteArray trGeneratedFromFiles(ClassDef::CompoundType compType, bool) {
      ...
      QByteArray result = (QByteArray)"此";
      switch (compType) {
         case ClassDef::Class:
            result += "類別(class)";
            break;
   ...
   virtual QByteArray trGeneratedFromFilesFortran(ClassDef::CompoundType compType,
      ...
      // single is true implies a single file
      QByteArray result = (QByteArray)"這個";
      switch (compType) {
         case ClassDef::Class:
            result += "模組";
            break;
   ...
   virtual QByteArray trEnumGeneratedFromFiles(bool single) {
      QByteArray result = "此列舉型態的文件是由下列檔案所產生";
      if (!single) {
         result += "";
      }
      result += ":";
   ...
   virtual QByteArray trServiceGeneratedFromFiles(bool) {
      // single is true implies a single file
      QByteArray result = (QByteArray)"本服務的文件由以下的檔案"
                          "所產生";
      result += ":";
      return result;
   }
   /** UNO IDL singleton page */
   virtual QByteArray trSingletonGeneratedFromFiles(bool) {
      // single is true implies a single file
      QByteArray result = (QByteArray)"本singleton的文件由下面的檔案"
                          "所產生";
      result += ":";
      return result;
   }
