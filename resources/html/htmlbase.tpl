...
<meta name="generator" content="DoxyPress {{ doxypress.version }}"/>
...
{% endif %}
<link href="{{ page.relPath }}{{ config.HTML_STYLESHEET|default:'doxypress.css' }}" rel="stylesheet" type="text/css" />
 ...
 {% if config.DISABLE_INDEX and config.SEARCHENGINE %}{# search box is part of title area #}
  ...
  {% if config.SERVER_BASED_SEARCH %}
   <div id="MSearchBox" class="MSearchBoxInactive">
    <div class="left">
     <form id="FSearchBox" action="{{ page.relPath }}{% if config.EXTERNAL_SEARCH %}search{{ doxypress.htmlFileExtension }}{% else %}search.php{% endif %}" method="get">
...
{% endblock %}
<!-- end header part -->
<!-- Generated by DoxyPress {{ doxypress.version }} -->
{% block search %}
...
{{ tr.generatedAt:doxypress.date,config.PROJECT_NAME }}
{% else %}
{{ tr.generatedby }}
{% endif %}
    <a href="http://www.copperspice.org/documentation-doxypress.html">
    <img class="footer" src="{{ page.relPath }}doxypress.png" alt="DoxyPress"/></a> {{ doxypress.version }} </li>
  </ul>
</div>
{% else %}
...
{{ tr.generatedby }}
{% endif %}
&#160;<a href="http://www.copperspice.org/documentation-doxypress.html"><img class="footer" src="{{ page.relPath }}doxypress.png" alt="DoxyPress"/></a> 
  {{ doxypress.version }}
  </small></address>
{% endif %}
</body>
